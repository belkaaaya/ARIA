...
public class Tartshttps://github.com/ESSTSup/Ekip2.5/blob/main/logout.php extends JFrame {

    private final Color PHONE_BG   = new Color(0xFBF1D6);
    private final Color TEXT_GOLD  = new Color(0x807043);
    private final Color SEARCH_BG  = new Color(0x241B02);
    ...
    public Tarts() {
        ...
        burger.setForeground(TEXT_GOLD);
        burger.setBorder(new EmptyBorder(0, 8, 0, 0));
        headerRow.add(burger, BorderLayout.WEST);
        ...
        // ================= TITLE =================
        JLabel pageTitle = new JLabel("WHIP & TARTS", SwingConstants.CENTER);
        ...
        listPanel.add(buildSavoryCard("strawberry.png", "STRAWBERRY CREAM",
                "Moelleux et doux, servis avec des fraises fraîches et un filet de sirop maison pour un moment gourmand.", 3.0));
        listPanel.add(Box.createVerticalStrut(12));

        listPanel.add(buildSavoryCard("blueberry.png", "BLUEBERRY THYME",
                "Pain brioché caramélisé, garni de myrtilles et de miel au thym pour une douceur fondante et parfumée..", 4.5));
        listPanel.add(Box.createVerticalStrut(12));

        listPanel.add(buildSavoryCard("kiwi.png", "KIWI VANILLA",
                "Crêpe fine et légère, généreusement garnie de kiwis frais et d’une touche de vanille pour un équilibre parfait", 5.0));
        listPanel.add(Box.createVerticalStrut(12));

        listPanel.add(buildSavoryCard("lemon.png", "LEMON CREAM",
                "Tarte dorée, garnie de crème au citron et d’un soupçon de miel pour plus de fraîcheur et de douceur.", 5.5));
        listPanel.add(Box.createVerticalStrut(12));

        listPanel.add(buildSavoryCard("vanilla.png", "VANILA CHOUX",
                "Un petit chou aérien au cœur de crème vanillée, surmonté de fruits frais et de granola croustillant", 7.0));
...
card.add(imgWrapper, BorderLayout.WEST);
   ...
   // cœur toggle + send to profile when liked
...
heartBtn.addActionListener(e -> {
    ...
    if (nowLike) {
        ...
        System.out.println("[Tarts] like " + title + " w=" +
...
private static ImageIcon scaleIcon(ImageIcon src, int w, int h) {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new Tarts().setVisible(true));
    }
}